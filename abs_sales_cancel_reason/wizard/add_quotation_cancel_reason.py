...
class AddQuotationCancelReason(models.TransientModel):
    ...
    def cancel_quotation(self):
        if self.env.context.get('active_model') == 'sale.order':
            ...
            sale_obj.user_cancel = self.create_uid

            sale_obj.write({'quota_cancel_reason_id':self.quota_cancel_reason_id.id, 'state':'cancel'})
            # if self.quota_cancel_reason_id.name :
            #           message.create({'model': 'sale.order', 'message_type': 'notification',
            #                 'body': "cancel Reason :" + self.quota_cancel_reason_id.name ,
            #                 'email_from': self.env.user.login, 'res_id': sale_obj.id})
