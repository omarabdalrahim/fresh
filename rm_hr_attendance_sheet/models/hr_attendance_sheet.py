...
class AttendanceSheet(models.Model):
    ...
    def action_approve(self):
        #ToDo Abdullah Nasser Asked To Not create Payslip when approve Payslip
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            for day in all_dates:
                ...
                reserved_intervals = []
                overtime_policy = policy_id.get_overtime(emp.contract_id.job_id.id)
                abs_flag = False
                if work_intervals:
                    if public_holiday:
                        if attendance_intervals:
                            for attendance_interval in attendance_intervals:
                                ...
                                print(">>>>>>ac_sign_out3", ac_sign_out)
                                values = {
                                    'date': date,
                                    'day': day_str,
                        ...
                        for i, work_interval in enumerate(work_intervals):
                            ...
                            if att_work_intervals:
                                if len(att_work_intervals) > 1:
                                    ...
                                    ac_sign_in = self._get_float_from_time(
                                        pytz.utc.localize(att_work_intervals[0][0]).astimezone(tz))
                                    ac_sign_out = self._get_float_from_time(pytz.utc.localize(att_work_intervals[0][1]).astimezone(tz))
                            ...
                            print(">>>>>>ac_sign_out11",ac_sign_out)
                            values = {
                                'date': date,
                                'day': day_str,
                        ...
                        if out_work_intervals:
                            for att_out in out_work_intervals:
                                ...
                                print(">>>>>>ac_sign_out5", ac_sign_out)
                                values = {
                                    'date': date,
                                    'day': day_str,
                                    'pl_sign_in': 0,
                    ...
                    if attendance_intervals:
                        ...
                        for attendance_interval in attendance_intervals:
                            ...
                            worked_hours = attendance_interval[1] - attendance_interval[0]
                            float_worked_hours = worked_hours.total_seconds() / 3600
                            print(">>>>>>ac_sign_out 22", ac_sign_out)
                            values = {
                                'date': date,
                                'day': day_str,
